...
pub mod screensaver;
pub mod settings;
pub mod storage_expansion;
pub mod theme_downloader;
pub mod update_checker;
pub mod wifi;

/// Decode artwork through the full image crate instead of Macroquad's limited
/// loader. Macroquad panics on unsupported JPEG input; this returns a normal
/// error so malformed or mislabeled removable-media artwork cannot terminate
/// the PlayFusion session and trigger Kazeta's shutdown fallback.
pub fn load_safe_image_texture(path: &Path) -> Result<Texture2D, String> {
...
pub fn render_ui_overlay_without_version(
    logo_cache: &HashMap<String, Texture2D>,
    font_cache: &HashMap<String, Font>,
    config: &Config,
    battery_info: &Option<BatteryInfo>,
...
fn render_ui_overlay_impl(
    ...
    // The PlayFusion wordmark is wider than the original Kazeta+ logo.
    const BASE_LOGO_WIDTH: f32 = 320.0;
    ...
    if config.logo_selection != "None" {
        if let Some(logo_to_draw) = logo_cache.get(&config.logo_selection) {
            ...
            // The PlayFusion source has transparent breathing room above the
            // artwork. A small negative offset places the visible mark close
            // to the top edge without clipping it.
            let y_pos = -15.0 * scale_factor;
    ...
    if show_version {
        ...
        let version_bottom_margin = 8.0 * scale_factor;
        ...
        let version_x = if config.menu_position == MenuPosition::BottomRight {
            ...
            screen_width() - version_dims.width - version_margin
        };
        ...
        text_with_config_color(
            ...
            VERSION_NUMBER,
            version_x,
            screen_height() - version_bottom_margin,
            font_size,
        );
    }
}
...
pub fn render_game_selection_menu(
    ...
    if let Some((cart_info, _)) = games.get(selected_game) {
        ...
        let text_y = screen_height() - (46.0 * scale_factor);

        text_with_config_color(font_cache, config, name, text_x, text_y, font_size);
    }

    let controls = "A PLAY   X INSTALL   B BACK";
...
pub fn text_with_config_color(
    ...
    if config.font_color == "PLAYFUSION" || config.font_color == "RAINBOW FUSION" {
        // Draw a readable animated neon gradient while preserving the caller's
        // measured starting position. This is deliberately resolution agnostic:
        // font_size already contains the active UI scale.
        let mut cursor_x = x;
        let time_phase = (get_time() as f32 * 0.055) + (x / screen_width().max(1.0));
        for (index, character) in text.chars().enumerate() {
            let glyph = character.to_string();
            let color = if config.font_color == "RAINBOW FUSION" {
                rainbow_fusion_color(time_phase + index as f32 * 0.028)
            } else {
                playfusion_neon_color(time_phase + index as f32 * 0.028)
            };
            draw_text_ex(
                &glyph,
                cursor_x,
            ...
            cursor_x += measure_text(&glyph, Some(font), font_size, 1.0).width;
        }
    } else {
...
/// PlayFusion neon palette used by the default typography and selection frames.
/// It follows the logo and Retro Laser Grid colors rather than a full rainbow.
/// `phase` wraps automatically, so callers can animate it with `get_time()`.
pub fn playfusion_neon_color(phase: f32) -> Color {
    const PALETTE: [Color; 8] = [
        Color::new(0.05, 0.88, 1.00, 1.0), // electric cyan
        Color::new(0.08, 0.46, 1.00, 1.0), // laser blue
        Color::new(0.38, 0.18, 1.00, 1.0), // ultraviolet
        Color::new(0.88, 0.10, 1.00, 1.0), // neon violet
        Color::new(1.00, 0.10, 0.70, 1.0), // PlayFusion magenta
        Color::new(1.00, 0.34, 0.12, 1.0), // logo orange accent
        Color::new(1.00, 0.10, 0.70, 1.0), // return through magenta
        Color::new(0.38, 0.18, 1.00, 1.0), // return through violet
    ];
...
/// Full-spectrum legacy effect retained as the optional Rainbow Fusion style.
pub fn rainbow_fusion_color(phase: f32) -> Color {
...
fn draw_animated_neon_frame(
    ...
    thickness: f32,
    intensity: f32,
    color_at: fn(f32) -> Color,
) {
    const SEGMENTS_PER_SIDE: usize = 6;
    let corners = [
        vec2(x, y),
        vec2(x + width, y),
    ...
    let travel = get_time() as f32 * 0.22;
    let alpha = intensity.clamp(0.0, 1.0);

    for side in 0..4 {
        let start = corners[side];
        let end = corners[side + 1];
        for segment in 0..SEGMENTS_PER_SIDE {
            let t0 = segment as f32 / SEGMENTS_PER_SIDE as f32;
            let t1 = (segment + 1) as f32 / SEGMENTS_PER_SIDE as f32;
            let p0 = start.lerp(end, t0);
            let p1 = start.lerp(end, t1);
            let mut color = color_at(travel + (side as f32 + t0) / 4.0);

            let mut glow = color;
            glow.a = 0.18 * alpha;
            draw_line(p0.x, p0.y, p1.x, p1.y, thickness * 2.8, glow);
...
/// Draws an animated PlayFusion frame whose colors travel clockwise.
pub fn draw_playfusion_neon_frame(
    x: f32,
    y: f32,
    width: f32,
    height: f32,
    ...
    draw_animated_neon_frame(
        ...
        thickness,
        intensity,
        playfusion_neon_color,
    );
}

pub fn draw_rainbow_fusion_frame(
    x: f32,
    y: f32,
    width: f32,
    height: f32,
    ...
    draw_animated_neon_frame(
        ...
        thickness,
        intensity,
        rainbow_fusion_color,
    );
}

/// Uses a branded animated frame for the two Fusion cursor choices.
/// Every other cursor-color setting retains the original animated solid frame.
pub fn draw_configured_cursor_frame(
    config: &Config,
    animation_state: &AnimationState,
    x: f32,
    y: f32,
    ...
    match config.cursor_color.as_str() {
        "PLAYFUSION" => {
            draw_playfusion_neon_frame(x, y, width, height, thickness, 1.0);
        }
        "RAINBOW FUSION" => {
            draw_rainbow_fusion_frame(x, y, width, height, thickness, 1.0);
        }
        _ => {
            draw_rectangle_lines(
                x,
...
/// A lighter-weight neon border for groups of non-selected data tiles.
pub fn draw_playfusion_panel_frame(
    x: f32,
    y: f32,
    width: f32,
    height: f32,
    ...
    let travel = get_time() as f32 * 0.08;
    let mut top = playfusion_neon_color(travel);
    let mut right = playfusion_neon_color(travel + 0.25);
    let mut bottom = playfusion_neon_color(travel + 0.5);
    let mut left = playfusion_neon_color(travel + 0.75);
    let alpha = intensity.clamp(0.0, 1.0);
    top.a = alpha;
    right.a = alpha;
    bottom.a = alpha;
    left.a = alpha;
    draw_line(x, y, x + width, y, thickness, top);
