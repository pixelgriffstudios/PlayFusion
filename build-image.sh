...
INCLUDE_PRIVATE_FIRMWARE=${INCLUDE_PRIVATE_FIRMWARE:-0}
PRIVATE_FIRMWARE_DIR=${PRIVATE_FIRMWARE_DIR:-private-firmware}
RUNTIME_BUNDLE_DIR=${RUNTIME_BUNDLE_DIR:-release-runtimes}
REQUIRE_RUNTIME_BUNDLE=${REQUIRE_RUNTIME_BUNDLE:-1}
...
install -Dm644 optical-stream/kazeta-cdstream.c \
    ${BUILD_PATH}/usr/src/playfusion/kazeta-cdstream.c
...
# PlayFusion public images are offline-complete: emulator runtimes are part of
# the OS image rather than optional post-install downloads. Keep the large KZR
# staging directory outside Git and pass RUNTIME_BUNDLE_DIR when necessary.
if [ -d "${RUNTIME_BUNDLE_DIR}" ] &&
   compgen -G "${RUNTIME_BUNDLE_DIR}/*.kzr" > /dev/null; then
	mkdir -p "${BUILD_PATH}/usr/share/kazeta/runtimes"
	rsync -a --delete \
		"${RUNTIME_BUNDLE_DIR}/" \
		"${BUILD_PATH}/usr/share/kazeta/runtimes/"
	echo "Bundled $(find "${RUNTIME_BUNDLE_DIR}" -maxdepth 1 -type f -name '*.kzr' | wc -l) internal runtimes."
elif [ "${REQUIRE_RUNTIME_BUNDLE}" = "1" ]; then
	echo "All-runtimes public build requested, but no KZR files were found in: ${RUNTIME_BUNDLE_DIR}" >&2
	exit 1
else
	echo "WARNING: building without bundled runtimes (REQUIRE_RUNTIME_BUNDLE=0)." >&2
fi

# Bundle PlayFusion's release games directly into the persistent internal
# library. Hell on Rails is a permanent first-party title, not test data.
if [ -d "release-bundle/internal-games" ]; then
	mkdir -p "${BUILD_PATH}/var/kazeta/internal-games"
	rsync -a \
		release-bundle/internal-games/ \
		"${BUILD_PATH}/var/kazeta/internal-games/"
	if [ -f "${BUILD_PATH}/var/kazeta/internal-games/internal-playfusion-arcade/game/playfusion-arcade" ]; then
		chmod 0755 "${BUILD_PATH}/var/kazeta/internal-games/internal-playfusion-arcade/game/playfusion-arcade"
	fi
	arch-chroot "${BUILD_PATH}" chown -R \
		"${USERNAME}:${USERNAME}" \
		/var/kazeta/internal-games
fi

# Bundle Jason's first-party PlayFusion music. These tracks are permanent
# release content and are available to the jukebox and music screensaver.
if [ -d "release-bundle/music" ]; then
	mkdir -p "${BUILD_PATH}/var/kazeta/music"
	rsync -a \
		release-bundle/music/ \
		"${BUILD_PATH}/var/kazeta/music/"
	arch-chroot "${BUILD_PATH}" chown -R \
		"${USERNAME}:${USERNAME}" \
		/var/kazeta/music
fi

# Media libraries are writable at runtime. Movies can be uploaded over FTP or
# installed from removable media; artwork metadata remains in a local cache.
mkdir -p \
	"${BUILD_PATH}/var/kazeta/movies" \
	"${BUILD_PATH}/var/kazeta/music" \
	"${BUILD_PATH}/var/kazeta/media-cache"
arch-chroot "${BUILD_PATH}" chown -R \
	kazetaftp:kazetaftp \
	/var/kazeta/movies \
	/var/kazeta/music \
	/var/kazeta/media-cache
find \
	"${BUILD_PATH}/var/kazeta/movies" \
	"${BUILD_PATH}/var/kazeta/music" \
	"${BUILD_PATH}/var/kazeta/media-cache" \
...
# Private/personal builds may carry firmware dumped by the owner plus emulator
# keys and caches. These files are deliberately external to the repository and
# are never included unless the builder explicitly opts in.
mkdir -p "${BUILD_PATH}/var/kazeta/firmware"
if [ "${INCLUDE_PRIVATE_FIRMWARE}" = "1" ]; then
	if [ ! -d "${PRIVATE_FIRMWARE_DIR}" ]; then
		echo "Private firmware was requested but the directory is missing: ${PRIVATE_FIRMWARE_DIR}" >&2
		exit 1
	fi
	rsync -a \
		"${PRIVATE_FIRMWARE_DIR}/" \
		"${BUILD_PATH}/var/kazeta/firmware/"
	arch-chroot "${BUILD_PATH}" chown -R \
		"${USERNAME}:kazetaftp" \
		/var/kazeta/firmware
	find "${BUILD_PATH}/var/kazeta/firmware" -type d -exec chmod 0755 {} +
	find "${BUILD_PATH}/var/kazeta/firmware" -type f -exec chmod 0644 {} +
	echo "Included owner-supplied private firmware and emulator keys."
else
	echo "Public build profile: private firmware and emulator keys were not included."
fi
