use crate::{
    ...
    text_with_config_color, ui::text_with_color, BackgroundState, InputState, Screen, VideoPlayer,
    DEV_MODE, FONT_SIZE,
};
        ...
        if let Ok(networks) = &self.networks {
            if let Some(selected_network) = networks.get(self.selected_index) {
                ...
                match output {
                    ...
                    Err(e) => {
                        self.screen_state =
                            WifiScreenState::Error(format!("Failed to run nmcli: {}", e));
                    }
                }
            }
        }
...
pub fn update(
    ...
    match &mut wifi_state.screen_state {
        WifiScreenState::PasswordInput => {
            ...
            let current_layout = if wifi_state.osk_shift_active {
                OSK_LAYOUT_UPPER
            } else {
                OSK_LAYOUT_LOWER
            };
            ...
            let current_physical_row_len = if *row < current_layout.len() {
                current_layout[*row].len()
            } else {
                OSK_SPECIAL_KEYS.len()
            };
            if *col >= current_physical_row_len {
                *col = current_physical_row_len - 1;
            }
            ...
            if input_state.select {
                ...
                if *row < current_layout.len() {
                    if let Some(key) = current_layout[*row].chars().nth(*col) {
                        ...
                        if wifi_state.osk_shift_active && *row > 0 {
                    ...
                    match OSK_SPECIAL_KEYS[*col] {
                        ...
                        "BACKSPACE" => {
                            wifi_state.password_buffer.pop();
                        }
            ...
            if let Ok(networks) = &wifi_state.networks {
                if let Some(network) = networks.get(wifi_state.selected_index) {
                    ...
                    draw_rectangle(
                        text_x,
                        input_box_y,
                        container_w - 80.0 * scale_factor,
                        input_box_height,
                        BLACK,
                    );
                    let text_y_inside_box = input_box_y
                        + (input_box_height / 2.0)
                        + (input_text_font_size as f32 / 2.5);
                    draw_text_ex(
                        &password_display,
                        text_x + 10.0 * scale_factor,
                        text_y_inside_box,
                        TextParams {
                            font: Some(font),
                    ...
                    // --- Special Keys Row ---
                    let special_row_y =
                        osk_start_y + (current_layout.len() as f32 * key_spacing) + 20.0;
                    let key_gap = 40.0 * scale_factor;
                    let mut total_row_width = 0.0;
                    for key_str in OSK_SPECIAL_KEYS.iter() {
                        total_row_width +=
                            measure_text(key_str, Some(font), osk_font_size, 1.0).width;
                    }
                    ...
                    let actual_key_gap = if total_row_width > available_width {
                        let text_width_sum: f32 = OSK_SPECIAL_KEYS
                            .iter()
                            .map(|k| measure_text(k, Some(font), osk_font_size, 1.0).width)
                            .sum();
                    ...
                    for (c, key_str) in OSK_SPECIAL_KEYS.iter().enumerate() {
                        ...
                        let is_active = (*key_str == "SHIFT" && wifi_state.osk_shift_active)
                            || (*key_str == "SHOW" && wifi_state.show_password);

                        let mut box_color = if is_active {
                ...
                Ok(networks) => {
                    if networks.is_empty() {
                        text_with_config_color(
                            ...
                            "No networks found.",
                            text_x,
                            container_y + 80.0 * scale_factor,
                            font_size,
                        );
                    } else {
                        for (i, ap) in networks.iter().take(10).enumerate() {
                            let y_pos =
                                container_y + 80.0 * scale_factor + (i as f32 * line_height * 1.5);
                            ...
                            text_with_config_color(
                                font_cache, config, &ap.ssid, text_x, y_pos, font_size,
                            );

                            let signal_text = format!("{}%", ap.signal_level);
                            let signal_dims =
                                measure_text(&signal_text, Some(font), font_size, 1.0);
                            let signal_x = container_x + container_w
                                - signal_dims.width
                                - (40.0 * scale_factor);
                            text_with_config_color(
                                font_cache,
                            ...
                            if !ap.security.is_empty() {
                                ...
                                text_with_config_color(
                                    font_cache, config, lock_text, lock_x, y_pos, font_size,
                                );
                            }
                        }
                    }
