...
use crate::{
    config::get_user_data_dir, types::StorageMedia, ui::profiles::active_profile_id, DEV_MODE,
};
...
pub fn find_all_game_files() -> Result<(Vec<PathBuf>, Vec<String>), SaveError> {
    ...
    debug_log.push(format!(
        "[Debug] Searching for .kzi and .kzp files in '{}' (max depth: 3)...",
        mount_dir
    ));

    // Search for both extensions
    match find_files_by_extension(mount_dir, &["kzi", "kzp"], 3, false) {
        Ok(files) => {
            let files = files
                .into_iter()
                .filter(|path| !is_managed_internal_media_path(path))
                .collect::<Vec<_>>();
            debug_log.push(format!(
...
fn is_managed_internal_media_path(path: &Path) -> bool {
    for media_root in [Path::new("/run/media"), Path::new("/media")] {
        let relative = match path.strip_prefix(media_root) {
            Ok(relative) => relative,
            Err(_) => continue,
        };
        let Some(mount_name) = relative.iter().next() else {
            continue;
        };
        if media_root
            .join(mount_name)
            .join(".playfusion-storage")
            .exists()
        {
            return true;
        }
...
pub fn parse_kzi_file(kzi_path: &Path) -> Result<CartInfo, SaveError> {
    ...
    if let (Some(id), Some(exec), Some(icon)) = (id, exec, icon) {
        ...
        Err(SaveError::Message(format!(
            "Invalid .kzi file: '{}'. Missing required fields.",
            kzi_path.display()
        )))
    }
}
...
pub fn get_save_dir_from_drive_name(drive_name: &str) -> String {
    ...
    if drive_name == "internal" || drive_name.is_empty() {
        let save_dir = if profile_id == "default" {
            base_dir.join("saves/default")
        } else {
            base_dir.join("saves/profiles").join(&profile_id)
        };
        if !save_dir.exists() {
        ...
        let save_dir = if profile_id == "default" {
            Path::new(&base_ext).join(drive_name).join("kazeta/saves")
        } else {
            Path::new(&base_ext)
                .join(drive_name)
                .join("kazeta/saves/profiles")
                .join(&profile_id)
        };
        if !save_dir.exists() {
            fs::create_dir_all(&save_dir).unwrap_or_else(|e| {
...
pub fn is_cart_connected() -> bool {
    if let Ok(files) = find_files_by_extension("/run/media", &["kzi", "kzp"], 3, false) {
        return files
            .iter()
            .any(|path| !is_managed_internal_media_path(path));
    }
...
pub fn get_save_details(drive_name: &str) -> io::Result<Vec<(String, String, String)>> {
    ...
    for entry in fs::read_dir(save_dir)? {
        ...
        // Prefer the cart's per-game cover in the save-data grid. The
        // platform icon is only a fallback for carts without cover art.
        let cache_path = Path::new(&cache_dir).join(cart_id);
        let icon_path = get_attribute(&metadata_path, "Cover")
            .ok()
            .and_then(|cover| {
                Path::new(&cover)
                    .file_name()
            ...
            .unwrap_or_else(|| cache_path.join("icon.png"));
