...
mkdir -p "$KIT_FULL_PATH/rootfs/usr/share/kazeta/optical-icons"
mkdir -p "$KIT_FULL_PATH/rootfs/usr/share/inputplumber/profiles"
mkdir -p "$KIT_FULL_PATH/optical-stream"
...
cp "$SOURCE_DIR/rootfs/etc/systemd/system/kazeta-cdstream@.service" "$KIT_FULL_PATH/rootfs/etc/systemd/system/"
cp "$SOURCE_DIR/rootfs/etc/systemd/system/playfusion-loose-rom.service" "$KIT_FULL_PATH/rootfs/etc/systemd/system/"
cp "$SOURCE_DIR/rootfs/etc/systemd/system/playfusion-loose-rom.path" "$KIT_FULL_PATH/rootfs/etc/systemd/system/"
cp "$SOURCE_DIR/rootfs/etc/systemd/system/playfusion-storage.service" "$KIT_FULL_PATH/rootfs/etc/systemd/system/"
...
scripts_to_copy=( "ethernet-connect" "kazeta" "kazeta-copy-logs" "kazeta-internal-game-helper" "kazeta-mount" "kazeta-optical-disc-helper" "kazeta-ps1-identify" "kazeta-runtime-helper" "kazeta-session" "kazeta-wifi-setup" "kazeta-show-error" "playfusion-bios-manager" "playfusion-loose-rom-helper" )
for script in "${scripts_to_copy[@]}"; do
    cp "$SOURCE_DIR/rootfs/usr/bin/$script" "$KIT_FULL_PATH/rootfs/usr/bin/"
done
cp "$SOURCE_DIR/optical-stream/kazeta-cdstream.c" "$KIT_FULL_PATH/optical-stream/"
cp "$SOURCE_DIR/rootfs/usr/share/kazeta/optical-icons/"*.png \
    "$KIT_FULL_PATH/rootfs/usr/share/kazeta/optical-icons/"
